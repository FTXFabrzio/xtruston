...
export class GoogleDriveService {
    ...
    private async initialize() {
        const credentialsJson = this.configService.get<string>('GSHEETS_CREDENTIALS_JSON'); // Reuse credentials? Assuming same service account
    ...
    async searchFiles(query: string): Promise<any[]> {
        ...
        try {
            ...
            this.logger.error(`Error searching drive files with query: ${query}`, error);
            return [];
        }
    }
}
